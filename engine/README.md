...
------------------------------------------------------------
競合A                       141   105    91          106   1.00
競合B                        197   164   136          159   0.94
競合C                       143   104    98          108   0.92
```
...
OTBはブッキングカーブ**から**生成されるため、実OTBと期待OTBの乖離は
構造的にゼロ近辺に留まります。**そのためペース関連の不具合は、
素のフィクスチャでは原理的に再現しません。**

実際にこれで取り逃しています。出所不明のブッキングカーブが進捗を常に
「大幅な遅れ」と判定していた件は、フィクスチャ上では症状が一切出ず、
実測相当のOTBを組み立てて初めて見えました。

そこで意図的な乖離を注入できます。

```bash
python3 scripts/make_fixtures.py --pace-divergence 0.3   # 想定の3割しか埋まらない
python3 scripts/make_fixtures.py --pace-divergence 2.5   # 想定の2.5倍で埋まる
python3 scripts/make_fixtures.py --no-scenarios          # 既定の乖離日を入れない
```

既定の生成にも乖離シナリオを2つ含めます。宿泊日を明示指定してあり、
実行のたびに変わりません（乱数で散らすとテストから名指しできず、
再現したい状況を狙って作れないため）。

| シナリオ | 宿泊日 | ベンチマーク比 | raw_z |
|---|---|---|---|
| `lagging` 想定より大きく遅れている | 2026-08-22 〜 08-28 | 0.30倍 | −0.93 〜 −0.62 |
| `leading` 想定より大きく先行している | 2026-08-29 〜 09-04 | 2.50倍 | +0.28 〜 +0.91 |

窓と倍率は「強く出るが、クリップに張り付かない」ところを総当たりで選んでいます。
張り付くとOTBが何室でも同じ値になり、進捗を見ていないのと同じになるためです。
張り付いて当然の極端な乖離は既定に入れず、`--pace-divergence` で明示的に作ります。

**リード45日以遠には乖離を置けません。** 実測カーブでは期待室数が0.1室未満になり、
倍率を掛けても四捨五入で消えるためです（0.05室 × 3.0 = 0.15室 → 0室）。
